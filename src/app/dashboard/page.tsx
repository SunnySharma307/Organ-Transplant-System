...
export default function Dashboard() {
  ...
  const [lastLoginTime, setLastLoginTime] = useState("Loading...");
  const [activeAllocationRows, setActiveAllocationRows] = useState<any[]>([]);
  ...
  const handleMatchRequest = async (match: any) => {
    try {
      if (!auth?.currentUser) {
      ...
      const requestData = {
        ...
        donor_organs: match.donor_organs || [],
        noisy_score: match.noisy_score || match.match_score || 0, // Fallback
        exact_score: match.exact_score || match.match_score || 0,
        prob_success: match.prob_success || 0,
        location: match.location || "Unknown",
        requested_by_uid: auth.currentUser.uid,
        requested_by_email: auth.currentUser.email || "unknown",
        requested_by_name: auth.currentUser.displayName || auth.currentUser.email || "Unknown User"
      };

      // Call Backend API instead of direct Firestore
      // Use existing API URL logic or relative path
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
      const res = await fetch(`${apiUrl}/match/request`, {
        method: 'POST',
        headers: {
      ...
      setRequestedMatches(prev => new Set(prev).add(match.donor_id));
  ...
  const fetchMatches = async () => {
    ...
    try {
      ...
      // 1. Get a valid recipient ID from our own API (Next.js)
      // This ensures we always try to match for a real patient in the DB
      ...
      if (!patients || patients.length === 0) {
        setMatchError("No patients found in database to match.");
        setMatchLoading(false);
        return;
      }

      // Pick the first patient (or one with high urgency)
      const targetPatient = patients[0];

      if (!targetPatient || !targetPatient.id) {
        setMatchError("No valid patients found. Please register a patient first.");
        setMatchLoading(false);
        console.warn("No valid patient data:", targetPatient);
        return;
      }

      const targetId = targetPatient.id; // Correct Firestore ID

      // 2. Call Matching API with real ID
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.detail || "Matching API error");
      }
  ...
  useEffect(() => {
    // Wait for auth to initialize or just capture mount time as "Session Start"
    // Since we don't have a full auth observer here ensuring non-null, we utilize the current session start.
    // Ideally: auth?.currentUser?.metadata.lastSignInTime
    const updateTime = () => {
      // Using current time as "Last Synced" since it represents when the dashboard loaded data
      setLastLoginTime(new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }));
    };
    updateTime();
    ...
    const fetchDonorCount = async () => {
      try {
        ...
        console.error("Error fetching donor count:", e);
      }
    };

    // Fetch patients for "Allocation Requests" simulation
    const fetchAllocations = async () => {
      try {
        // Call Python Backend
        const res = await fetch('http://localhost:8000/match/allocations');
        const data = await res.json();
        setActiveAllocationRows(data);
      } catch (e) { console.error("FastAPI Error:", e); }
    };
    fetchDonorCount();
    fetchAllocations();
    fetchMatches();
  ...
  >({});
  const [donorFormData, setDonorFormData] = useState<any>({});
  ...
  const handlePatientSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/patients/register', {
        ...
        body: JSON.stringify({
          ...patientFormData,
          organRequired: recipientOrganRequired,
        }),
      });
      ...
      alert("Error registering patient: " + (error.message || error));
    } finally {
  ...
  const handleDonorSubmit = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      const selectedOrgans = Object.keys(donorOrgansSelected).filter(k => donorOrgansSelected[k]);
      ...
      alert("Error registering donor: " + (error.message || error));
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <div className="p-6 overflow-y-auto">
              {registerStep === 1 ? (
                <form
                  ...
                  onSubmit={(e) => {
                    e.preventDefault();
                    // Validate basic fields here if needed
                    // For now, just accumulating data is done via onChange on inputs
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setPatientFormData({ ...patientFormData, abhaId: e.target.value })}
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          className="w-full border border-slate-300 p-2 text-sm rounded-sm bg-white outline-none font-bold text-slate-700"
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          className="w-full border border-slate-300 p-2 text-sm rounded-sm bg-white outline-none"
                          onChange={(e) => setPatientFormData({ ...patientFormData, urgencyStatus: e.target.value })}
                        >
                            ...
                            >
                              ...
                              <div className="flex-1 flex gap-2 flex-wrap">
                                <input
                                  ...
                                  placeholder="Value"
                                  onChange={(e) => setPatientFormData((prev: any) => ({ ...prev, [`test_${organKey}_${t.key}`]: e.target.value }))}
                                />
                                <input
                                  type="date"
                                  ...
                                  placeholder="Date"
                                  onChange={(e) => setPatientFormData((prev: any) => ({ ...prev, [`test_${organKey}_${t.key}_date`]: e.target.value }))}
                                />
                              </div>
                            </div>
                          ))}
              ...
              {donorStep === 1 ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <select
                          className="w-full border border-slate-300 p-2 text-sm rounded-sm bg-white outline-none focus:border-green-800 focus:ring-1 focus:ring-green-800"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full border border-slate-300 p-2 text-sm rounded-sm outline-none"
                          placeholder="XX-XXXX-XXXX-XXXX"
                          onChange={(e) => setDonorFormData({ ...donorFormData, abhaId: e.target.value })}
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-slate-700 mb-1">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setDonorFormData({ ...donorFormData, nextOfKin: e.target.value })}
                      />
                    </div>
                  </div>
                ...
                >
                                  ...
                                  >
                                    ...
                                    <div className="flex-1 flex gap-2 flex-wrap">
                                      <input
                                        ...
                                        placeholder="Value"
                                        onChange={(e) => setDonorFormData((prev: any) => ({ ...prev, [`test_${groupKey}_${t.key}`]: e.target.value }))}
                                      />
                                      <input
                                        type="date"
                                        ...
                                        className="border border-slate-300 p-2 text-sm rounded-sm outline-none w-[140px]"
                                        placeholder="Date"
                                        onChange={(e) => setDonorFormData((prev: any) => ({ ...prev, [`test_${groupKey}_${t.key}_date`]: e.target.value }))}
                                      />
                                    </div>
                                  </div>
                                ))}
                  ...
                  <div className="bg-slate-50 p-4 border-t border-slate-200 flex justify-end gap-3 rounded-b-sm -mx-6 -mb-6 px-6">
                    ...
                    >
                      <Save size={16} />
                      {isSubmitting ? "Submitting..." : "Submit to Donor Registry"}
                    </button>
                  </div>
                </form>
              )}
        ...
        <main className="flex-1 overflow-y-auto p-6 z-0">
          {activeSection === "overview" && (
            <>
              <div className="flex justify-between items-center mb-8">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    Last Synced: Today, {lastLoginTime}
                  </p>
                </div>
              ...
              {/* Compatible Donor Matches Section */}
              <section className="mt-8 bg-white p-6 rounded-lg shadow-md border border-slate-200">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold text-slate-800">Compatible Donor Matches</h2>
                  <button
                    onClick={() => fetchMatches()}
                    className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 font-medium text-sm flex items-center gap-2"
                  >
                    Refresh Matches
                  </button>
                </div>

                {matchLoading && <p className="text-center py-4 text-slate-500">Loading matches...</p>}
                {matchError && <p className="text-red-600 text-center py-4">{matchError}</p>}
                ...
                {!matchLoading && !matchError && (
                  matches.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-200 text-sm">
                        <thead className="bg-gray-50">
                          <tr>
                            <th className="px-6 py-3 text-left font-bold text-gray-500 uppercase">Donor ID</th>
                            <th className="px-6 py-3 text-left font-bold text-gray-500 uppercase">Organ(s)</th>
                            <th className="px-6 py-3 text-left font-bold text-gray-500 uppercase">Noisy Score</th>
                            <th className="px-6 py-3 text-left font-bold text-gray-500 uppercase">Success Prob</th>
                            <th className="px-6 py-3 text-left font-bold text-gray-500 uppercase">Location</th>
                            <th className="px-6 py-3 text-left font-bold text-gray-500 uppercase">Privacy</th>
                            <th className="px-6 py-3 text-left font-bold text-gray-500 uppercase">Action</th>
                          </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                          {matches.map((match: any) => (
                            <tr key={match.donor_id} className="hover:bg-slate-50">
                              <td className="px-6 py-4 whitespace-nowrap text-slate-700 font-mono">Donor #{match.donor_id}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-slate-700 font-medium">{match.donor_organs && match.donor_organs.length > 0 ? match.donor_organs.join(", ") : "N/A"}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-purple-600 font-bold">{match.noisy_score}%</td>
                              <td className="px-6 py-4 whitespace-nowrap text-slate-700">{match.prob_success}%</td>
                              <td className="px-6 py-4 whitespace-nowrap text-slate-700">{match.location}</td>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                {requestedMatches.has(match.donor_id) ? (
                                  <span className="text-green-600 font-bold">Request Sent</span>
                                ) : (
                                  <button
                                    onClick={() => handleMatchRequest(match)}
                                    className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded font-bold text-sm transition-colors"
                                  >
                                    Request
                                  </button>
                                )}
                    ...
                    <p className="text-center text-gray-600 py-6">No matches found. Try another patient.</p>
                  )
                )}
              </section>
            </>
          ...
          {activeSection === "organ-inventory" && (
            <OrganInventoryView lastLoginTime={lastLoginTime} />
          )}
...
function PatientWaitlistView({
  ...
  const filteredRows = waitlistRows.filter((row) =>
    row.id.toLowerCase().includes(appliedQuery.toLowerCase()) ||
    ...
    async function fetchData() {
      try {
        ...
        const mapped = data.map((item) => ({
          id: `P-${item.hospitalId?.split("-")[1] || item.id.slice(0, 4)}`,
          name: item.fullName,
          organ: item.organRequired,
          bloodGroup: item.bloodGroup,
          urgency: item.urgencyStatus || (item.urgency_score > 80 ? "Super Urgent" : "Elective"),
          status: item.status?.charAt(0).toUpperCase() + item.status?.slice(1) || "Active",
          listedDate: item.registeredAt ? new Date(item.registeredAt).toLocaleDateString() : "Recently"
        }));
        setWaitlistRows(mapped);
      } catch (err) {
        console.error(err);
      }
    }
    ...
    <>
      ...
      <div className="bg-white border border-slate-200 rounded-sm shadow-sm">
              ...
              {filteredRows.map((row) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded text-xs font-bold ${row.urgency === "Super Urgent"
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-bold ${row.status === "Active"
        ...
        <div className="px-6 py-3 border-t border-slate-200 bg-slate-50 text-xs text-slate-500 flex justify-between">
          <span>
            Showing 1–{filteredRows.length} of {waitlistRows.length} records
          </span>
...
function OrganInventoryView({ lastLoginTime }: { lastLoginTime?: string }) {
  ...
  const filteredRows = inventoryRows.filter((row) =>
    row.id.toLowerCase().includes(appliedQuery.toLowerCase()) ||
    row.organType.toLowerCase().includes(appliedQuery.toLowerCase())
  );
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Set the actual donor count from database
        setDonorCount(data.length);

        // Flatten donors -> organs
        const flattenedInv: any[] = [];
        data.forEach(donor => {
          const organs = donor.organsWillingToDonate || [];
          organs.forEach((organ: string) => {
            flattenedInv.push({
              id: `ORG-${donor.hospitalId?.split('-')[1] || donor.id.slice(0, 4)}-${organ.substring(0, 2).toUpperCase()}`,
              organType: organ,
              bloodGroup: donor.bloodGroup,
              donorType: donor.donorType || "Deceased",
              source: "In-house",
              status: "Available",
              retrievedAt: donor.registeredAt ? new Date(donor.registeredAt).toLocaleDateString() : "Today"
            });
          });
        });

        setInventoryRows(flattenedInv);
      } catch (err) {
        console.error(err);
      }
    }
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm p-4">
          ...
          <p className="text-2xl font-bold text-green-700 mt-1">{donorCount}</p>
        ...
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm p-4">
          ...
          <p className="text-lg font-bold text-slate-800 mt-1">{lastLoginTime}</p>
      ...
      <div className="bg-white border border-slate-200 rounded-sm shadow-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRows.map((row) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded text-xs font-bold ${row.source === "In-house"
                        ? "bg-blue-100 text-blue-800"
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-bold ${row.status === "Available"
                        ? "bg-green-100 text-green-800"
                        : row.status === "Matching"
                          ? "bg-amber-100 text-amber-800"
                          : "bg-slate-100 text-slate-700"
        ...
        <div className="px-6 py-3 border-t border-slate-200 bg-slate-50 text-xs text-slate-500 flex justify-between">
          <span>
            Showing 1–{filteredRows.length} of {inventoryRows.length} records
          </span>
          <div className="flex gap-2">
            <button
              className="hover:text-blue-700 font-bold disabled:opacity-50"
...
function AllocationStatusView() {
  ...
  const [acceptedRows, setAcceptedRows] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const requestsRef = firestoreCollection(db, 'matches'); // Changed from 'requests'
        ...
        const requests = snapshot.docs.map((doc, index) => {
          ...
          // score_display -> score
          // timestamp -> responseDue
          // best_match_donor_id -> donorId

          return {
            id: doc.id,
            docId: doc.id,
            organType: data.organ || 'N/A',
            patient: data.patient_name || data.patient_details || 'Unknown',
            score: data.score_display || `Score: ${Math.round((data.match_score || 0) * 100)}%`,
            status: data.status || 'Pending',
            statusColor: data.statusColor || 'bg-amber-100 text-amber-800',
            responseDue: data.timestamp ? new Date(data.timestamp).toLocaleString() : 'N/A',
            donorId: data.best_match_donor_id,
            fullData: data
          };
        });

        setAllocationRows(requests);
      } catch (error) {
    ...
    const fetchAcceptedRequests = async () => {
      try {
        ...
        const { getDocs, collection: firestoreCollection } = await import('firebase/firestore');
        const acceptedRef = firestoreCollection(db, 'requests_accepted');
        const snapshot = await getDocs(acceptedRef);

        const accepted = snapshot.docs.map((doc) => {
          const data = doc.data();
          return {
            id: doc.id.substring(0, 12).toUpperCase(),
            organType: data.donor_organs?.join(', ') || 'N/A',
            patient: data.requested_by_name || 'Unknown',
            score: `Score: ${Math.round(data.noisy_score * 100)}%`,
            donorId: data.donor_id,
            acceptedAt: new Date(data.accepted_at).toLocaleString(),
            requestedAt: new Date(data.requested_at).toLocaleString()
          };
        });
  ...
  const handleAcceptRequest = async (requestId: string, requestData: any) => {
    try {
      if (!auth?.currentUser) {
      ...
      // Call Backend API instead of direct Firestore
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
      const res = await fetch(`${apiUrl}/match/accept`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Optionally refresh the accepted list using fetchAcceptedRequests() if accesible, 
      // or just trust the backend. For now we just alert.

      alert("Request accepted successfully!");
    } catch (error: any) {
      console.error("Error accepting request:", error);
      alert(`Failed to accept request: ${error?.message || error?.toString() || 'Unknown error'}`);
    }
  };
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm p-4">
          ...
          <p className="text-2xl font-bold text-amber-700 mt-1">
            {allocationRows.filter(r => r.status === 'pending').length.toString().padStart(2, '0')}
          </p>
        ...
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm p-4">
          ...
          <p className="text-2xl font-bold text-green-700 mt-1">
            {allocationRows.filter(r => r.status === 'accepted').length.toString().padStart(2, '0')}
          </p>
        ...
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm p-4">
          ...
          <p className="text-2xl font-bold text-blue-700 mt-1">
            {allocationRows.length.toString().padStart(2, '0')}
          </p>
        ...
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm p-4">
          ...
          <p className="text-lg font-bold text-slate-800 mt-1">{new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}</p>
          <p className="text-xs text-slate-500">IST</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-sm shadow-sm">
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="text-center py-8 text-slate-500">No requests found. Click "Request" on a donor match to create one.</div>
          ) : (
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-100 text-slate-600 font-bold uppercase text-xs">
                <tr>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      {row.status === 'accepted' ? (
                        ...
                        <button
                          onClick={() => handleAcceptRequest(row.docId, row.fullData)}
                          style={{ backgroundColor: '#16a34a', color: 'white' }}
                          className="px-4 py-2 rounded font-bold text-sm shadow-md hover:opacity-90 transition-opacity"
                        >
                          Accept
                        </button>
                      )}
      ...
      <div className="bg-white border border-slate-200 rounded-sm shadow-sm mt-8">
        <div className="px-6 py-4 border-b border-slate-200 bg-green-50">
          <h3 className="font-bold text-slate-800">✅ Accepted Requests</h3>
          <p className="text-sm text-slate-600 mt-1">Requests that have been accepted and are being processed</p>
        </div>
        <div className="overflow-x-auto">
          {acceptedLoading ? (
            <div className="text-center py-8 text-slate-500">Loading...</div>
          ) : acceptedRows.length === 0 ? (
            <div className="text-center py-8 text-slate-500">No accepted requests yet.</div>
          ) : (
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-100 text-slate-600 font-bold uppercase text-xs">
                <tr>
                  <th className="px-6 py-3 border-b border-slate-200">ID</th>
                  <th className="px-6 py-3 border-b border-slate-200">Organ</th>
                  <th className="px-6 py-3 border-b border-slate-200">Requested By</th>
                  <th className="px-6 py-3 border-b border-slate-200">Score</th>
                  <th className="px-6 py-3 border-b border-slate-200">Donor</th>
                  <th className="px-6 py-3 border-b border-slate-200">Accepted At</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {acceptedRows.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4 font-mono text-slate-700">{row.id}</td>
                    <td className="px-6 py-4 font-medium">{row.organType}</td>
                    <td className="px-6 py-4">{row.patient}</td>
                    <td className="px-6 py-4">{row.score}</td>
                    <td className="px-6 py-4 font-mono">#{row.donorId}</td>
                    <td className="px-6 py-4 text-green-600 font-bold text-xs">{row.acceptedAt}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        if (!db) return;
        const { getDocs, collection: firestoreCollection } = await import('firebase/firestore');
        const docsRef = firestoreCollection(db, 'documents');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const docData = {
        ...
        uploadedByName: auth.currentUser.displayName || auth.currentUser.email || 'Unknown User',
        downloadURL: downloadURL,
        storagePath: `reports/${fileName}`
      };

      const docRef = await addDoc(firestoreCollection(db!, 'documents'), docData);

      // Update local state
      setUploadedDocs(prev => [...prev, { id: docRef.id, ...docData }]);
      ...
      {isUploadModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <h3 className="text-xl font-bold text-slate-800 mb-4">Submit New Report</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-slate-300 rounded-sm focus:outline-none focus:border-blue-500"
                >
                  <option value="">Select report type...</option>
                  <option value="Form 10 (BSD Certification)">Form 10 (BSD Certification)</option>
                  <option value="Form 12 (Registration Renewal)">Form 12 (Registration Renewal)</option>
                  <option value="Monthly Transplant Summary">Monthly Transplant Summary</option>
                  <option value="Quarterly Waitlist Update">Quarterly Waitlist Update</option>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                style={{ backgroundColor: uploading ? '#9ca3af' : '#1e40af' }}
                className="flex-1 text-white px-4 py-2 rounded-sm font-medium hover:opacity-90 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={uploading}
                className="flex-1 bg-slate-200 text-slate-700 px-4 py-2 rounded-sm font-medium hover:bg-slate-300 disabled:cursor-not-allowed"
              >
                Cancel
              </button>
            </div>
